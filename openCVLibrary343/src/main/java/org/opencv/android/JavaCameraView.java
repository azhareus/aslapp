        ...
        synchronized (this) {
            ...
            if (mCameraIndex == CAMERA_ID_ANY) {
                ...
                if (mCamera == null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                    ...
                    for (int camIdx = 0; camIdx < Camera.getNumberOfCameras(); ++camIdx) {
                        Log.d(TAG,
                                "Trying to open camera with new open(" + Integer.valueOf(camIdx) + ")");
                        try {
                            mCamera = Camera.open(camIdx);
                            connected = true;
                        } catch (RuntimeException e) {
                            Log.e(TAG,
                                    "Camera #" + camIdx + "failed to open: " + e.getLocalizedMessage());
                        }
                        if (connected) break;
                    }
                }
            ...
            try {
                ...
                if (sizes != null) {
                    ...
                    Size frameSize = calculateCameraFrameSize(sizes, new JavaCameraSizeAccessor()
                            , width, height);
                    ...
                    if (Build.FINGERPRINT.startsWith("generic")
                        ...
                        params.setPreviewFormat(ImageFormat.YV12);  // "generic" or "android" =
                        // android emulator
                    else
                    ...
                    mScale = Math.max(((float) height) / mFrameHeight,
                    ...
                    mCameraFrame[0] = new JavaCameraFrame(mFrameChain[0], mFrameWidth,
                            mFrameHeight);
                    mCameraFrame[1] = new JavaCameraFrame(mFrameChain[1], mFrameWidth,
                            mFrameHeight);
